...
**
** enta_wp_title()
** Customizes Theme Tile
**
***********************************************************/

function enta_wp_title( $title, $sep ) {
  global $paged, $page;
  ...
  if ( $paged >= 2 || $page >= 2 )
    $title = "$title $sep " . sprintf( __( 'Page %s', 'enta' ), max( $paged, $page ) );

  return $title;
}
add_filter( 'wp_title', 'enta_wp_title', 10, 2 );
...
**
** register_sidebar()
** Registers widgetized sidebar areas
**
***********************************************************/
...
if ( function_exists('register_sidebar') ) {
    ...
    register_sidebar(array(
        ...
        'id' => 'page-sidebar',
        'description' => __( 'Widgets in this area will be shown on the right-hand side of a "Page".' ),
   ...
   register_sidebar(array(
        ...
        'id' => 'post-sidebar',
        'description' => __( 'Widgets in this area will be shown on the right-hand side of a "Post"' ),
...
** Modifies acf custom fields save point for local json / sync json
**
***********************************************************/
...
** Modifies acf custom fields load point for local json / sync json
**
***********************************************************/
...
**
** acf_update_setting()
** Updates Google API Key for Maps
**
***********************************************************/
function my_acf_init() {
    
    acf_update_setting('google_api_key', 'AIzaSyCFJXi-fYW1dsXn3qVz87UQBhcxWrHSjWk');
}
...
**
** add_shortcode()
** Functions for adding shortcodes. Used for universal
** shortcodes in the content editor.
**
***********************************************************/
function universal_phone_number( $atts ){
    return get_field('phone_number', 'option');
}
add_shortcode( 'phonenumber', 'universal_phone_number' );
add_shortcode( 'phone_number', 'universal_phone_number' );
...
**
** custom_breadcrumbs()
** Functions for breadcrumbs on pages, posts, etc
**
***********************************************************/
function custom_breadcrumbs() {
    ...
    if ( !is_front_page() ) {
        ...
        // Home page
        echo '<li class="item-home" itemprop="itemListElement" itemtype="http://schema.org/ListItem"><a class="bread-link bread-home" href="' . get_home_url() . '" title="' . $home_title . '"><i class="fa fa-home"></i></a></li>';
